...
export const drag_and_drop = tool({
  description:
  ...
  args: {
    pairs: tool.schema
      .array(
        tool.schema.object({
          sourceSelector: tool.schema.string().optional(),
          sourceText: tool.schema.string().optional(),
          sourceIndex: tool.schema.number().optional(),
          targetSelector: tool.schema.string().optional(),
          targetText: tool.schema.string().optional(),
          targetIndex: tool.schema.number().optional(),
        })
      )
      .optional()
      .describe("Optional list of drag pairs to run in order."),
    sourceSelector: tool.schema
      .string()
      .optional()
      .describe("CSS selector for the draggable source element."),
    sourceText: tool.schema
      .string()
      .optional()
      .describe("Visible text for the draggable source element."),
    sourceIndex: tool.schema
      .number()
      .optional()
      .describe("Zero-based index to disambiguate source matches."),
    targetSelector: tool.schema
      .string()
      .optional()
      .describe("CSS selector for the drop target element."),
    targetText: tool.schema
      .string()
      .optional()
      .describe("Visible text for the drop target element."),
    targetIndex: tool.schema
      .number()
      .optional()
      .describe("Zero-based index to disambiguate target matches."),
    strategy: tool.schema
      .enum(["auto", "react", "dragTo", "mouse", "dataTransfer"])
      .optional()
      .describe("DnD strategy. 'auto' tries react → dataTransfer → mouse → dragTo. Default: auto."),
    steps: tool.schema
      .number()
      .optional()
      .describe("Mouse move steps (for mouse strategy). Default 12."),
    timeoutMs: tool.schema
      .number()
      .optional()
      .describe("Timeout for each strategy attempt (ms). Default 3000."),
  },
  async execute(args) {
    ...
    if (!args.pairs && !args.sourceSelector && !args.sourceText) {
      return "Error: provide pairs or sourceSelector/sourceText and targetSelector/targetText"
    }
        ...
        const result = await page.evaluate(
          async ({ pairs, delayMs }) => {
            const findElement = (
              ...
              if (text) {
                // For source: look for draggable elements
                // For target: look for drop zone elements
